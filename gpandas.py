...
def plot_zoomed_map(lat_zoom, lon_zoom, zoom_scale=5):
    # Load the netCDF file
    dataset = nc.Dataset(r"/mnt/c/Users/ASUS/Desktop/terraspatial/ISRO_immersion_data/soildata/LPRM-AMSR2_01T_L3_SM_D20240711.nc")
    ...
    llcrnrlat = lat_zoom - zoom_scale
    urcrnrlat = lat_zoom + zoom_scale
    llcrnrlon = lon_zoom - zoom_scale
    urcrnrlon = lon_zoom + zoom_scale

    # Set up Basemap for the zoomed area
    m = Basemap(projection='merc',
    ...
    # Add a marker for the zoom location
    x_marker, y_marker = m(lon_zoom, lat_zoom)
    m.plot(x_marker, y_marker, 'ko', markersize=5)
    plt.text(x_marker, y_marker, '  User Location', fontsize=12, ha='left', va='center', color='black')
    ...
    # Add labels and title
    plt.title('Surface Soil Moisture with Zoomed Area')
    plt.show()


# Example usage
plot_full_map()
plot_zoomed_map(lat_zoom=20.5937, lon_zoom=78.9629, zoom_scale=1)